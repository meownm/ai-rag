...
class AnswerRequest(BaseModel):
    ...
    stream: bool = Field(default=False, description="Если true, ответ будет передаваться потоком через SSE.")
    mode: Literal["dense", "bm25", "hybrid", "graph", "hybrid+graph"] = Field(default="hybrid", description="Режим поиска.")
    context_mode: Literal["short", "long"] = Field(default="long", description="Режим контекста (пока не используется).")
    graph_depth: int = Field(default=2, description="Глубина обхода графа знаний.")
    top_k: int = Field(default=10, description="Количество наиболее релевантных чанков для извлечения.")
    filters: Optional[Filters] = None
    max_tokens: int = Field(default=2048, description="Максимальное количество токенов для сгенерированного ответа.")
