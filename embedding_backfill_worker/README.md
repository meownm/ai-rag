...
- **Модель**: берётся из `settings` (`embedding_config`). Поддерживаются режимы `local` (SentenceTransformer) и `api` (совместимый с OpenAI `/embeddings`). Для API-режима можно задать `api_base` в конфиге или переменной `EMBEDDING_API_BASE`.
