...
class EmbeddingConfig:
    ...
    generator: str = "service"  # service | ollama | local
...
class EmbeddingBackfillWorker:
    ...
    def _load_config(self, conn) -> EmbeddingConfig:
        ...
        generator = raw_value.get("generator") or os.getenv("EMBEDDING_GENERATOR") or ("service" if mode == "api" else "local")
    ...
    def _ensure_model(self, config: EmbeddingConfig) -> Any:
        ...
        if config.mode == "api" or config.generator in {"service", "ollama"}:
            ...
            logger.info("Загружена конфигурация API эмбеддингов (%s): %s", config.generator, api_base)
            return self._model
    ...
    def _embed_batch(self, texts: Sequence[str], model: Any, config: EmbeddingConfig) -> List[List[float]]:
        ...
        if isinstance(model, dict) and model.get("mode") == "api":
            ...
            generator = model.get("generator", "service")

            if generator == "ollama":
                endpoint = f"{api_base.rstrip('/')}/api/embeddings"
                embeddings: List[List[float]] = []
                for text in texts:
                    response = requests.post(endpoint, json={"model": model["model_name"], "prompt": text}, timeout=60)
